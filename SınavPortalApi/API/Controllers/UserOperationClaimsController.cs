...
namespace WebAPI.Controllers
{
    ...
    public class UserOperationClaimsController : ControllerBase
    {
        ...
        [HttpGet("get-dto-user-operation-claims")]
        public IActionResult GetUserDtoOperationClaims()
        {
            var result = UserOperationClaimService.GetUserDtoOperationClaims();
            if (result.Status)
            {
                return Ok(result);
            }
        ...
        [HttpGet("get-dto-by-user-email/{userEmail}")]
        public IActionResult GetDtoByUserId(string userEmail)
        {
            var result = UserOperationClaimService.GetDtoByUserEmail(userEmail);
            if (result.Status)
            {
                return Ok(result);
            }
